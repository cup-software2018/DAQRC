...
def handle_requests():
    ...
    Clients include Run Control (RC) and TSDB agents.
    """
                ...
                with db_write_lock:
                    with sqlite3.connect(onlconsts.kRUNCATALOGDBFILE, timeout=5.0) as db_conn:
                        ...
                        if cmd == "BOOT_RUN":
                            ...
                            log.info(
                                "BOOT_RUN processed. Assigned Run Number: %d", response["run_num"])
...
def run_monitor():
    """
    Monitor DAQ state and perform real-time DB tracking at 1Hz.
    DB writes are serialized via db_write_lock to prevent conflicts
    with handle_requests thread.
    """
    ...
    while True:
        ...
        try:
            ...
            if reply is None:
                log.debug("DAQ State Reply is None from server: %s",
                          onlconsts.kDAQSERVER_ADDR)
                try:
               ...
               onlutils.check_state(run_state, onlconsts.kRUNENDED):
                ...
                if current_run_number != last_run_number:
                    if last_run_number != -1:
                        log.info("New Run %d detected. Re-initializing monitoring modules.",
                                 current_run_number)
                    ...
                    try:
                        with db_write_lock:
                            with sqlite3.connect(onlconsts.kRUNCATALOGDBFILE, timeout=5.0) as conn:
                                cursor = conn.cursor()
                                cursor.execute(
                                    update_query, tuple(update_params))
                                conn.commit()
